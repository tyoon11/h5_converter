"""
HEEDB 변환 유틸리티
- signal_statistics: per-lead 통계
- beat_similarity: per-lead correlation + DTW
- extract_beat_annotation: neurokit2 R-peak
- extract_fiducial: neurokit2 fiducial point + feature
- reorder_signal: sig_name → TARGET 순서 reorder + transpose
"""
...
def reorder_signal(signal, actual_sig_name):
    """
    signal: (samples, n_leads) from WFDB
    actual_sig_name: record.sig_name
    returns: (n_leads, samples) float16 — reordered + transposed
    """
...
def has_zero_lead(signal):
    """signal: (samples, n_leads) — 전체 0인 리드가 1개라도 있으면 True"""
...
def signal_statistics(signal):
    ...
    returns: dict with per-lead lists
    """
...
def beat_similarity(signal, sampling_rate=500):
    """
    signal: (samples, n_leads)
    returns: dict with per-lead lists (bs_corr, bs_dtw)
    """
    try:
