...
from clinical_ts.models.basic_conv1d import AdaptiveConcatPool1d, create_head1d
...
class Inception1d(nn.Module):
    ...
    def __init__(self, num_classes=2, input_channels=8, kss=[39,19,9], depth=6, bottleneck_size=32, nb_filters=32, use_residual=True,lin_ftrs_head=None, ps_head=0.5, bn_head=True, act_head="relu", concat_pooling=True):
        super().__init__()
        ...
        n_ks = len(kss) + 1
        #head
        head = create_head1d(n_ks*nb_filters, nc=num_classes, lin_ftrs=lin_ftrs_head, ps=ps_head, bn=bn_head, act=act_head, concat_pooling=concat_pooling)
        layers.append(head)
